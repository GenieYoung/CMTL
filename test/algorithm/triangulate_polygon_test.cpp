...
void test1() {
  ...
  CMTL::algorithm::triangulate_polygon_2d(polygon, clip_triangles);
  CMTL::io::write_obj(CMTL::geo2d::PolygonSoup<double>(points, clip_triangles),
                      "test1_output.obj");
  std::cout << "test1 : " << std::endl;
  for (unsigned i = 0; i < clip_triangles.size(); ++i) {
    std::cout << clip_triangles[i][0] << " " << clip_triangles[i][1] << " "
              << clip_triangles[i][2] << " " << std::endl;
  }
}
...
void test3() {
  ...
  points.push_back(CMTL::geo2d::Point<double>{0, 1});
  std::vector<std::array<unsigned int, 3>> clip_triangles;
  CMTL::algorithm::triangulate_polygon_2d(points, clip_triangles);
...
void test4() {
  ...
  std::vector<std::array<unsigned int, 3>> clip_triangles;
  CMTL::algorithm::triangulate_polygon_3d(polygon, clip_triangles);
...
void test5() {
  ...
  points.push_back(CMTL::geo3d::Point<double>{0, 1, 0});
  std::vector<std::array<unsigned int, 3>> clip_triangles;
  CMTL::algorithm::triangulate_polygon_3d<double>(points, clip_triangles);
