...
TEST(SurfaceMesh2Test, ConstructTest) {
  ...
  EXPECT_TRUE(sm.halfedge_handle(fhs[0]) == hes[0] ||
              sm.halfedge_handle(fhs[0]) == hes[2] ||
              sm.halfedge_handle(fhs[0]) == hes[4]);
  EXPECT_TRUE(sm.halfedge_handle(fhs[1]) == hes[3] ||
              sm.halfedge_handle(fhs[1]) == hes[8] ||
              sm.halfedge_handle(fhs[1]) == hes[7]);
  EXPECT_TRUE(sm.halfedge_handle(fhs[2]) == hes[5] ||
              sm.halfedge_handle(fhs[2]) == hes[6] ||
              sm.halfedge_handle(fhs[2]) == hes[10]);
...
void test1() {
  ...
  std::cout << sm.n_vertices() << " " << sm.n_edges() << " " << sm.n_faces()
            << std::endl;
  std::cout << sm.degree(v0) << std::endl;
  unsigned vf_count = 0;
  for (auto vf = sm.vf_begin(v0); vf != sm.vf_end(v0); ++vf) vf_count++;
...
void test2() {
  ...
  for (auto eit = sm.edges_begin(); eit != sm.edges_end(); ++eit) {
    if (sm.is_flip_ok(*eit)) sm.flip(*eit);
  }
...
void test3() {
  Surface_mesh sm;
  CMTL::io::read_obj(
      sm, "/data/home/yangjinlin/work/code/geometry/mesh_data/leaf.obj");
  CMTL::io::write_obj(sm, "geo2d_surface_mesh_test2.obj");
}
