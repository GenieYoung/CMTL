...
class Plane {
 ...
 public:
  ...
  Plane(const Point<T>& p, const Point<T>& n) : _origin(p), _normal(n) {
    ...
    _params[3] = -(_params[0] * _origin[0] + _params[1] * _origin[1] +
                   _params[2] * _origin[2]);
    _params[4] = (max_abs > 0 ? T(1) : T(-1));

    // build the coordinate plane which has the most similar normal, used for
    // projecting between 2d and 3d
    if (max_abs_id == 0)
      _project_cood = {1, 2, 0};
   ...
   */
  Plane(const Point<T>& p0, const Point<T>& p1, const Point<T>& p2)
 ...
 public:
  /**
   * @brief get a point on plane
   */
  const Point<T>& origin() const { return _origin; }

  /**
   * @brief get the plane normal
   */
  const Point<T>& normal() const { return _normal; }

  /**
   * @brief project a plane point to a local 2d coordinate system,
   *        the 2d coordinate system is a plane parallel with xy/xz/yz
   * coordinate plane, decide by _project_normal
   */
  ...
  /**
   * @brief project a 2d point on the local coordinate system to origin
   * coordinate system
   */
   ...
   * @param consider_orientation if true, same plane with different orientation
  ...
  bool is_equal(const Plane& other, bool consider_orientation = false) const {
    if (consider_orientation)
      ...
      return std::equal(_params.begin(), _params.end() - 1,
                        other._params.begin());
  }
  ...
  /**
   * @brief less comparator, used for sorting, same plane with different
   * orientation will be considered as different
   */
 ...
 public:
  ...
  /* normalized parameters of plane function, the last parameter indicates
   * whether the plane is reversed */
  std::array<T, 5> _params;

  /* auxiliary vector used for tranform from 2d coordinate system and 3d
   * coordinate system */
  std::array<unsigned, 3> _project_cood;

  /* the normal of project coordinate plane, used for projecting between 2d and
   * 3d */
