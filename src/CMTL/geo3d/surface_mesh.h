...
/**
 * @brief surface mesh traits, user specified traits should inherit this class
 * and override these members.
 */
 ...
 * @tparam Traits element attributes, user specified traits should inherit
 * SurfaceMeshTraits and override those members.
 */
...
class SurfaceMesh : public halfedge::Graph<geo3d::Point<T>, Traits> {
 ...
 public:
   ...
   */
  bool is_triangle_mesh() const { return this->has_constant_face_degree(3); }
  ...
  Point normal(FaceHandle fh) const {
    ...
    for (; first_v != this->fv_end(fh); ++first_v, ++second_v) {
      x += ((this->point(*first_v).z() + this->point(*second_v).z()) *
            (this->point(*first_v).y() - this->point(*second_v).y()));
      y += ((this->point(*first_v).x() + this->point(*second_v).x()) *
            (this->point(*first_v).z() - this->point(*second_v).z()));
      z += ((this->point(*first_v).y() + this->point(*second_v).y()) *
            (this->point(*first_v).x() - this->point(*second_v).x()));
    }
    return Point(x, y, z);
  }
};
