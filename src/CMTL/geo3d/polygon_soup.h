#ifndef CMTL_GEO3D_POLYGON_SOUP_H
#define CMTL_GEO3D_POLYGON_SOUP_H
...
class PolygonSoup {
 ...
 public:
   ...
   * @note point container must have iterator method and operator[] method to
   * get coordinate, face container must have iterator method.
   */
 ...
 public:
   ...
   */
  size_t n_points() const { return _vertices.size(); }

  /**
   * @brief return the number of polygons
   */
  size_t n_polygons() const { return _polygons.size(); }

  /**
   * @brief get the ith point
   */
  Point<T>& point(unsigned i) { return _vertices[i]; }

  /**
   * @brief get the const ith point
   */
  const Point<T>& point(unsigned i) const { return _vertices[i]; }
...
#endif  // CMTL_GEO3D_POLYGON_SOUP_H