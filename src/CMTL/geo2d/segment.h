...
class Segment {
 ...
 public:
   ...
   */
  Segment(const Point<T>& p0, const Point<T>& p1) : _first(p0), _second(p1) {}
 ...
 public:
   ...
   */
  Point<T>& first() { return _first; }

  /**
   * @brief get the const first point
   */
  const Point<T>& first() const { return _first; }

  /**
   * @brief get the writable second point
   */
  Point<T>& second() { return _second; }

  /**
   * @brief get the const second point
   */
  const Point<T>& second() const { return _second; }

  /**
   * @brief return the direction of the segment
   */
  Point<T> direction() const { return _second - _first; }
...
class Line {
 ...
 public:
   ...
   */
  Point<T>& direction() { return _direction; }

  /**
   * @brief get the const direction
   */
  const Point<T>& direction() const { return _direction; }
