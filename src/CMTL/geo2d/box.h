...
class Box {
 ...
 public:
   ...
   */
  Point<T> left_top() const { return Point<T>(left(), top()); }

  /**
   * @brief get the right bottom point
   */
  Point<T> right_bottom() const { return Point<T>(right(), bottom()); }
   ...
   */
  T width() const { return top() - bottom(); }

  /**
   * @brief return the area of the box
   */
  T area() const { return length() * width(); }
