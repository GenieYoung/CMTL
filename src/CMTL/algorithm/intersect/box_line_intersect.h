...
template <typename T>
bool intersect(const geo2d::Box<T>& box, const geo2d::Point<T>& ori,
               const geo2d::Point<T>& dir, T*& t0, T*& t1) {
  ...
  if (p1 != 0) {
    ...
    T q2_p2 = q2 / p2;
    if (p1 > 0) {
      if ((t0 && q1_p1 < *t0) || (t1 && q2_p2 > *t1)) return false;
      ...
      if ((t0 && q2_p2 < *t0) || (t1 && q1_p1 > *t1)) return false;
  ...
  if (p3 != 0) {
    ...
    T q4_p4 = q4 / p4;
    if (p3 > 0) {
      if ((t0 && q3_p3 < *t0) || (t1 && q4_p4 > *t1)) return false;
      ...
      if ((t0 && q4_p4 < *t0) || (t1 && q3_p3 > *t1)) return false;
 ...
 */
template <typename T>
bool intersect(const geo2d::Box<T>& box, const geo2d::Segment<T>& segment,
               T& t0, T& t1) {
  T* t_min = new T(0);
  T* t_max = new T(1);
  bool result =
      intersect(box, segment.first(), segment.direction(), t_min, t_max);
  if (result) {
    t0 = *t_min;
    t1 = *t_max;
  }
 ...
 */
template <typename T>
bool intersect(const geo2d::Segment<T>& segment, const geo2d::Box<T>& box,
               T& t0, T& t1) {
 ...
 */
template <typename T>
bool intersect(const geo2d::Box<T>& box, const geo2d::Line<T>& line, T& t0,
               T& t1) {
  T* t_min = nullptr;
  T* t_max = nullptr;
 ...
 */
template <typename T>
bool intersect(const geo2d::Line<T>& line, const geo2d::Box<T>& box, T& t0,
               T& t1) {
 ...
 */
template <typename T>
bool intersect(const geo2d::Box<T>& box, const geo2d::Ray<T>& ray, T& t0,
               T& t1) {
  T* t_min = new T(0);
  T* t_max = nullptr;
 ...
 */
template <typename T>
bool intersect(const geo2d::Ray<T>& ray, const geo2d::Box<T>& box, T& t0,
               T& t1) {
