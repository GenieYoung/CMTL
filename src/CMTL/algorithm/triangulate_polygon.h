...
class triangulate_polygon_modifier_2d {
 public:
  triangulate_polygon_modifier_2d(const Polygon& polygon) : _polygon(polygon) {}
 ...
 public:
  bool execute(std::vector<std::array<unsigned, 3>>& triangles) {
    ...
    if (n_points == 3) {
      triangles = std::vector<std::array<unsigned, 3>>{
          std::array<unsigned, 3>{0, 1, 2}};
      return true;
    }
    _vertex_list.resize(n_points);
    for (int i = 0, j = n_points - 1, k = 1; i < n_points;
         ++i, ++j, ++k, j %= n_points, k %= n_points) {
      vertex v;
      v.prev_id = j;
      v.next_id = k;
      v.cur_id = i;
    ...
    for (unsigned i = 0; i < n_points; ++i) {
      if (_vertex_list[i].is_convex) check_ear(_vertex_list[i]);
    }
    unsigned begin_id = 0;
    unsigned iter_times = 0;
    triangles.reserve(n_points - 2);
    while (_vertex_list[_vertex_list[begin_id].next_id].next_id !=
           _vertex_list[begin_id].prev_id) {
      ...
      while (!_vertex_list[first_ear].is_ear) {
        ...
        if (++iter_times_inner > _vertex_list.size()) return false;
      }
      triangles.push_back(
          std::array<unsigned, 3>{_vertex_list[first_ear].prev_id, first_ear,
                                  _vertex_list[first_ear].next_id});
      _vertex_list[_vertex_list[first_ear].prev_id].next_id =
          _vertex_list[first_ear].next_id;
      _vertex_list[_vertex_list[first_ear].next_id].prev_id =
          _vertex_list[first_ear].prev_id;
      ...
      if (++iter_times > _vertex_list.size()) return false;
    }
    triangles.push_back(
        std::array<unsigned, 3>{_vertex_list[begin_id].prev_id, begin_id,
                                _vertex_list[begin_id].next_id});
    return true;
  }
 ...
 private:
  void re_configure(vertex& v) {
    if (v.is_convex || v.is_ear) {
      ...
      check_concavity(v);
      if (v.is_convex) check_ear(v);
    }
  }

  void check_concavity(vertex& v) {
    ORIENTATION orientation =
        orient_2d(_polygon[v.prev_id], _polygon[v.cur_id], _polygon[v.next_id]);
  ...
  void check_ear(vertex& v) const {
    ...
    while (loc != v.prev_id) {
      ...
      if (orient_2d(_polygon[v.prev_id], _polygon[v.next_id], _polygon[loc]) !=
...
/**
 * @brief triangulate a single counterclock-wise 2d-polygon into several
 ...
 *       polygon struct should have [] operator to get the point and the size()
 * method to get the length; the point struct should have [] opeartor to get the
 * coordinate.
 */
template <typename Polygon>
bool triangulate_polygon_2d(const Polygon& polygon,
...
class triangulate_polygon_modifier_3d {
 ...
 private:
  ...
  void check_concavity(vertex& v) {
    double orientation = ((_polygon[v.cur_id] - _polygon[v.prev_id]) %
                          (_polygon[v.next_id] - _polygon[v.cur_id]))
                             .dot(_normal);
  ...
  void check_ear(vertex& v) const {
    ...
    while (loc != v.prev_id) {
      ...
      if (((_polygon[loc] - _polygon[v.prev_id]) %
           (_polygon[v.next_id] - _polygon[v.prev_id]))
              .dot(_normal) >= 0) {
        v.is_ear = false;
        return;
      }
...
/**
 * @brief triangulate a single 3d-polygon into several triangles using
 * ear-clipping
 * @return true if the polygon be trianguled successfully.
 * @note the polygon should be intersect-free.
 */
template <typename T>
bool triangulate_polygon_3d(const geo3d::Polygon<T>& polygon,
                            std::vector<std::array<unsigned, 3>>& triangles) {
...
bool triangulate_polygon_3d(const Polygon& polygon,
                            std::vector<std::array<unsigned, 3>>& triangles) {
