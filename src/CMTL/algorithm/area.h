...
void area_2d(const Point& p1, const Point& p2, const Point& p3,
             NumberType& result) {
  result =
 ...
 * @note  polygon struct should have [] operator to get the point and the size()
...
void area_2d(ForwardIterator begin, ForwardIterator end, NumberType& result) {
  ...
  ++second;
  if (second == end) return;
  while (first != end) {
    result += (first->operator[](0) * second->operator[](1) -
               second->operator[](0) * first->operator[](1));
    ++first;
    ++second;
    if (second == end) second = begin;
  }
  result /= NumberType(2);
}
