...
class ElemHandle {
 public:
  ...
  bool is_valid() const { return _idx >= 0; }

  /** @brief make the handle invalid */
  void invalidate() { _idx = -1; }

  bool operator==(const ElemHandle& other) const { return _idx == other._idx; }
...
/**
 * @brief element handle connect to the topology graph, allow access to the
 * graph
...
class GraphElemHandle {
 public:
  ...
  const GraphTopology* graph() const { return _graph; }
...
class GraphHalfedgeHandle : public GraphElemHandle, public HalfedgeHandle {
 public:
  explicit GraphHalfedgeHandle(int idx = -1,
                               const GraphTopology* graph = nullptr)
...
class IteratorBase {
 public:
  ...
  IteratorBase(const Topo* topo, ElemHandle eh)
      : _geh(GraphElemHandle(eh.idx(), topo)) {}

  /** @brief dereferencing opeartor */
  const GraphElemHandle& operator*() const { return _geh; }
...
/**
 * @brief base iterator structure used for interating all the vertices around a
 * vertex
 * @tparam Topo topology graph
...
class VertexVertexIterBase {
 public:
  ...
  VertexVertexIterBase(const Topo* topo, VertexHandle vh, bool end = false)
      : _topo(topo),
        _start(_topo->halfedge_handle(vh)),
        _heh(_start),
        _cycle_count(static_cast<int>(end)) {
    // if(CCW)
    // {
    //     int cc = _cycle_count;
    //     ++(*this);
    ...
    if (!_start.is_valid()) _cycle_count = 0;
  }
 ...
 public:
  ...
  bool is_valid() const { return _heh.is_valid() && _cycle_count == 0; }

  bool operator==(const VertexVertexIterBase& other) const {
    return _topo == other._topo && _start == other._start &&
           _heh == other._heh && _cycle_count == other._cycle_count;
  }
...
/**
 * @brief base iterator structure used for interating all the halfedges around a
 * vertex
 * @tparam Topo topology graph
 * @tparam CCW countor-clock-wise order if true, otherwise clock-wise
 */
...
/**
 * @brief base iterator structure used for interating all the edges around a
 * vertex
 * @tparam Topo topology graph
 * @tparam CCW countor-clock-wise order if true, otherwise clock-wise
 */
...
/**
 * @brief base iterator structure used for interating all the faces around a
 * vertex
 * @tparam Topo topology graph
 * @tparam CCW countor-clock-wise order if true, otherwise clock-wise
 */
...
class VertexFaceIterBase {
 public:
  ...
  VertexFaceIterBase(const Topo* topo, VertexHandle vh, bool end = false)
    ...
    if (_heh.is_valid() && !_topo->face_handle(_heh).is_valid() &&
        _cycle_count == 0)
      ++(*this);
  }
...
class FaceVertexIterBase {
 public:
  ...
  FaceVertexIterBase(const Topo* topo, FaceHandle fh, bool end = false)
      : _topo(topo),
        _start(_topo->halfedge_handle(fh)),
        _heh(_start),
        _cycle_count(static_cast<int>(end)) {
    // if(CCW)
    // {
...
typedef IteratorBase<GraphTopology, HalfedgeHandle, GraphHalfedgeHandle>
    HalfedgeIter;
...
class GraphTopology {
 ...
 public:
  ...
  unsigned n_vertices() const { return _vertices.size(); }

  /** @brief get the number of edges */
  unsigned n_edges() const { return _edges.size(); }

  /** @brief get the number of vertices */
  unsigned n_halfedges() const { return _edges.size() * 2; }

  /** @brief get the number of faces */
  unsigned n_faces() const { return _faces.size(); }
 ...
 public:
  ...
  EdgeIter edges_begin() { return EdgeIter(this, EdgeHandle(0)); }
  ...
  EdgeIter edges_end() { return EdgeIter(this, EdgeHandle(int(n_edges()))); }
  ...
  FaceIter faces_begin() { return FaceIter(this, FaceHandle(0)); }
  ...
  FaceIter faces_end() { return FaceIter(this, FaceHandle(int(n_faces()))); }
  ...
  VertexFaceIter vf_begin(VertexHandle vh) { return VertexFaceIter(this, vh); }
  ...
  FaceVertexIter fv_begin(FaceHandle fh) { return FaceVertexIter(this, fh); }
 ...
 public:
  ...
  bool is_boundary(EdgeHandle eh) const {
    return is_boundary(halfedge_handle(eh, 0)) ||
           is_boundary(halfedge_handle(eh, 1));
  }

  /** @brief number of vertices around given vertex */
  unsigned degree(VertexHandle vh) const {
    unsigned count(0);
    for (auto vv = vv_begin(vh); vv != vv_end(vh); ++vv) ++count;
    return count;
  }
  ...
  unsigned degree(FaceHandle fh) const {
    unsigned count(0);
    for (auto fv = fv_begin(fh); fv != fv_end(fh); ++fv) ++count;
    return count;
  }
 ...
 public:
  ...
  bool is_flip_ok(EdgeHandle eh) const {
    ...
    if (next_halfedge_handle(next_halfedge_handle(h0)) !=
            prev_halfedge_handle(h0) ||
        next_halfedge_handle(next_halfedge_handle(h1)) !=
            prev_halfedge_handle(h1))
      return false;
  ...
  void flip(EdgeHandle eh) {
    ...
    if (halfedge_handle(v0) == a0) vertex_item(v0)._halfedge_handle = b1;
    if (halfedge_handle(v1) == b0) vertex_item(v1)._halfedge_handle = a1;
  }

  /** @brief flip an edge */
  void flip(HalfedgeHandle heh) { flip(edge_handle(heh)); }
  ...
  VertexHandle split_edge(EdgeHandle eh, bool split_face = false) {
    if (!eh.is_valid()) return VertexHandle();
    ...
    if (halfedge_handle(v1) == he1) vertex_item(v1)._halfedge_handle = new_he1;

    if (split_face) {
      if (f0.is_valid()) {
        std::vector<VertexHandle> connect_vertices;
        for (auto he = next_halfedge_handle(new_he0);
             he != prev_halfedge_handle(he0); he = next_halfedge_handle(he)) {
      ...
      if (f1.is_valid()) {
        std::vector<VertexHandle> connect_vertices;
        for (auto he = next_halfedge_handle(he1);
             he != prev_halfedge_handle(new_he1);
  ...
  EdgeHandle split_face(FaceHandle fh, VertexHandle v0, VertexHandle v1) {
    if (!fh.is_valid() || !v0.is_valid() || !v1.is_valid()) return EdgeHandle();
    ...
    if (!a0.is_valid()) return EdgeHandle();
    ...
    if (!b0.is_valid()) return EdgeHandle();
  ...
  bool is_collapse_ok(HalfedgeHandle heh) const { return false; }
 ...
 public:
  ...
  GraphHalfedgeHandle find_halfedge(VertexHandle start, VertexHandle end) {
    ...
    for (ConstVertexOHalfedgeIter voh = voh_begin(start); voh.is_valid();
         ++voh) {
      if (to_vertex_handle(*voh) == end) return *voh;
    }
 ...
 public:
   ...
   */
  GraphVertexHandle add_vertex() { return vertex(new_vertex()); }

  /**
   * @brief add a 3-degree face into graph and build connectivity information
   * @return the new face handle
   */
  GraphFaceHandle add_face(VertexHandle vh0, VertexHandle vh1,
                           VertexHandle vh2) {
  ...
  GraphFaceHandle add_face(const std::vector<VertexHandle>& vhs) {
    ...
    for (unsigned i = 0, j = 1; i < n; ++i, ++j, j %= n) {
      ...
      _tmp_edge_storage[i].is_new =
          !_tmp_edge_storage[i].halfedge_handle.is_valid();
      if (!_tmp_edge_storage[i].is_new &&
          !is_boundary(_tmp_edge_storage[i].halfedge_handle)) {
    ...
    for (unsigned i = 0, j = 1; i < n; ++i, ++j, j %= n) {
      if (!_tmp_edge_storage[i].is_new && !_tmp_edge_storage[j].is_new) {
        ...
        if (next_halfedge_handle(inner_prev_he) != inner_next_he) {
          HalfedgeHandle outer_prev_he =
              opposite_halfedge_handle(inner_next_he);
          HalfedgeHandle outer_next_he =
              opposite_halfedge_handle(inner_prev_he);
          HalfedgeHandle boundary_prev_he = outer_prev_he;
          do {
            boundary_prev_he = opposite_halfedge_handle(
                next_halfedge_handle(boundary_prev_he));
          } while (!is_boundary(boundary_prev_he));
          HalfedgeHandle boundary_next_he =
              next_halfedge_handle(boundary_prev_he);
          ...
          _he_link_storage[link_count++] =
              std::make_pair(boundary_prev_he, patch_start_he);
          _he_link_storage[link_count++] =
              std::make_pair(patch_end_he, boundary_next_he);
          _he_link_storage[link_count++] =
              std::make_pair(inner_prev_he, inner_next_he);
        }
      }
    }
    ...
    for (unsigned i = 0, j = 1; i < n; ++i, ++j, j %= n) {
      ...
      if (flag) {
        ...
        switch (flag) {
          case 1: {
            HalfedgeHandle boundary_prev_he =
                prev_halfedge_handle(inner_next_he);
            assert(boundary_prev_he.is_valid());
            _he_link_storage[link_count++] =
                std::make_pair(boundary_prev_he, outer_next_he);
            vertex_item(vh)._halfedge_handle = outer_next_he;
            break;
          }
          case 2: {
            HalfedgeHandle boundary_next_he =
                next_halfedge_handle(inner_prev_he);
            assert(boundary_next_he.is_valid());
            _he_link_storage[link_count++] =
                std::make_pair(outer_prev_he, boundary_next_he);
          ...
          case 3: {
            if (!halfedge_handle(vh).is_valid()) {
              vertex_item(vh)._halfedge_handle = outer_next_he;
              _he_link_storage[link_count++] =
                  std::make_pair(outer_prev_he, outer_next_he);
            } else {
              HalfedgeHandle boundary_next_he = halfedge_handle(vh);
              HalfedgeHandle boundary_prev_he =
                  prev_halfedge_handle(boundary_next_he);
              assert(boundary_next_he.is_valid());
              assert(boundary_prev_he.is_valid());
              _he_link_storage[link_count++] =
                  std::make_pair(boundary_prev_he, outer_next_he);
        ...
        _tmp_edge_storage[j].need_adjust =
            (halfedge_handle(vh) == inner_next_he);
      }
    ...
    for (unsigned i = 0; i < link_count; ++i) {
      halfedge_item(_he_link_storage[i].first)._next_halfedge_handle =
          _he_link_storage[i].second;
      halfedge_item(_he_link_storage[i].second)._prev_halfedge_handle =
          _he_link_storage[i].first;
    }

    for (unsigned i = 0; i < n; ++i) {
      if (_tmp_edge_storage[i].need_adjust) adjust_outgoing_halfedge(vhs[i]);
    }
 ...
 public:
  ...
  void print() {
    ...
    for (unsigned i = 0; i < n_vertices(); ++i) {
      printf("%-8d%d\n", i,
             vertex_item(vertex_handle(i))._halfedge_handle.idx());
    }

    printf("halfedge from_v to_v next_he prev_he oppo_he face\n");
    for (unsigned i = 0; i < n_halfedges(); ++i) {
      printf("%-9d%-7d%-5d%-8d%-8d%-8d%d\n", i,
             from_vertex_handle(halfedge_handle(i)).idx(),
             to_vertex_handle(halfedge_handle(i)).idx(),
             halfedge_item(halfedge_handle(i))._next_halfedge_handle.idx(),
             halfedge_item(halfedge_handle(i))._prev_halfedge_handle.idx(),
             opposite_halfedge_handle(halfedge_handle(i)).idx(),
             halfedge_item(halfedge_handle(i))._face_handle.idx());
    }
  }
 ...
 private:
  ...
  /* container that store the information used for next halfedge link when add a
   * new face */
  std::vector<std::pair<HalfedgeHandle, HalfedgeHandle> > _he_link_storage;

  /** @brief if the vertex has a boundary outgoing halfedge around it, link the
   * halfedge */
  void adjust_outgoing_halfedge(VertexHandle vh) {
    for (ConstVertexOHalfedgeIter voh_it = voh_begin(vh); voh_it != voh_end(vh);
         ++voh_it) {
...
inline GraphHalfedgeHandle GraphVertexHandle::halfedge() const {
  return GraphHalfedgeHandle(this->graph()->halfedge_handle(*this).idx(),
                             this->graph());
}
...
inline GraphHalfedgeHandle GraphEdgeHandle::halfedge(unsigned i) const {
  return GraphHalfedgeHandle(this->graph()->halfedge_handle(*this, i).idx(),
                             this->graph());
}
...
inline GraphHalfedgeHandle GraphHalfedgeHandle::next() const {
  return GraphHalfedgeHandle(this->graph()->next_halfedge_handle(*this).idx(),
                             this->graph());
}

inline GraphHalfedgeHandle GraphHalfedgeHandle::prev() const {
  return GraphHalfedgeHandle(this->graph()->prev_halfedge_handle(*this).idx(),
                             this->graph());
}

inline GraphHalfedgeHandle GraphHalfedgeHandle::opposite() const {
  return GraphHalfedgeHandle(
      this->graph()->opposite_halfedge_handle(*this).idx(), this->graph());
}

inline GraphVertexHandle GraphHalfedgeHandle::from_vertex() const {
  return GraphVertexHandle(this->graph()->from_vertex_handle(*this).idx(),
                           this->graph());
}

inline GraphVertexHandle GraphHalfedgeHandle::to_vertex() const {
  return GraphVertexHandle(this->graph()->to_vertex_handle(*this).idx(),
                           this->graph());
}

inline GraphEdgeHandle GraphHalfedgeHandle::edge() const {
  return GraphEdgeHandle(this->graph()->edge_handle(*this).idx(),
                         this->graph());
}

inline GraphFaceHandle GraphHalfedgeHandle::face() const {
  return GraphFaceHandle(this->graph()->face_handle(*this).idx(),
                         this->graph());
}
...
/**
 * @brief base class for all traits, user traits should be derived from this
 * class and override these traits
 */
...
/**
 * @brief A graph describe the topological relationship and mesh items of the
...
class Graph : public GraphTopology {
 ...
 public:
  ...
  Point& point(VertexHandle vh) {
    assert(vh.is_valid() && vh.idx() < n_vertices());
    if (vh.idx() >= _points.size()) _points.resize(vh.idx() + 1);
  ...
  VertexAttribute& attribute(VertexHandle vh) {
    assert(vh.is_valid() && vh.idx() < n_vertices());
    if (vh.idx() >= _vertex_attr.size()) _vertex_attr.resize(vh.idx() + 1);
  ...
  EdgeAttribute& attribute(EdgeHandle eh) {
    assert(eh.is_valid() && eh.idx() < n_edges());
    if (eh.idx() >= _edge_attr.size()) _edge_attr.resize(eh.idx() + 1);
  ...
  FaceAttribute& attribute(FaceHandle fh) {
    ...
    if (fh.idx() >= _face_attr.size()) _face_attr.resize(fh.idx() + 1);
